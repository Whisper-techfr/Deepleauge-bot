...
async def init_db():
    async with aiosqlite.connect(DB) as db:
        ...
        # ── Guild dodge tracking ──────────────────────────────────────────────
        # Each guild has a hard cap of 8 total dodges (DODGE_LIMIT).
        # Once exhausted the Dodge button is disabled — they must accept wars.
        await db.execute("""
            CREATE TABLE IF NOT EXISTS guild_dodge_counts (
        ...
        # Records last user-message time and reminder / extension state for the
        # background task that handles 3-day fight pings and auto-close logic.
        await db.execute("""
            CREATE TABLE IF NOT EXISTS ticket_activity (
                ...
                last_fight_ping_at TIMESTAMP,
                extension_offered_at TIMESTAMP,
                extension_expires_at TIMESTAMP
            )
        """)
...
async def get_guild_by_member(user_id, server_id):
    """Returns any guild the user is in (pending OR accepted)."""
    async with aiosqlite.connect(DB) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("""
...
async def get_guild_by_member_accepted(user_id, server_id):
    """Returns only guilds where the user has fully accepted (not pending invites)."""
    async with aiosqlite.connect(DB) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("""
            SELECT g.* FROM guilds g
            JOIN guild_members gm ON g.id = gm.guild_id
            WHERE gm.user_id=? AND g.server_id=? AND gm.accepted=1
        """, (user_id, server_id)) as cursor:
            return await cursor.fetchone()
...
# ── DODGE TRACKING FUNCTIONS ─────────────────────────────────────────────────

DODGE_LIMIT = 8  # Hard cap: a guild can dodge at most 8 wars ever


async def get_guild_dodge_count(guild_id: int) -> int:
    """Returns the total number of times this guild has dodged a war."""
    async with aiosqlite.connect(DB) as db:
        async with db.execute(
            "SELECT dodge_count FROM guild_dodge_counts WHERE guild_id=?", (guild_id,)
        ) as cursor:
            row = await cursor.fetchone()
            return row[0] if row else 0


async def increment_guild_dodge(guild_id: int) -> int:
    """
    Increments this guild's dodge counter and returns the new total.
    Row is created automatically on first dodge.
    """
    async with aiosqlite.connect(DB) as db:
        await db.execute("""
            INSERT INTO guild_dodge_counts (guild_id, dodge_count)
            VALUES (?, 1)
            ON CONFLICT(guild_id) DO UPDATE SET dodge_count = dodge_count + 1
        """, (guild_id,))
        await db.commit()
        async with db.execute(
            "SELECT dodge_count FROM guild_dodge_counts WHERE guild_id=?", (guild_id,)
...
async def guild_can_dodge(guild_id: int) -> tuple:
    """
    Returns (can_dodge: bool, dodges_remaining: int).
    can_dodge is False when the guild has used all DODGE_LIMIT dodges.
    """
    count = await get_guild_dodge_count(guild_id)
    remaining = max(0, DODGE_LIMIT - count)
    return remaining > 0, remaining


# ── TICKET ACTIVITY FUNCTIONS ─────────────────────────────────────────────────

async def register_ticket_activity(channel_id: int, ticket_type: str, ticket_id: int):
    """Called when a ticket channel is first created to begin tracking."""
    async with aiosqlite.connect(DB) as db:
        await db.execute("""
            INSERT OR REPLACE INTO ticket_activity
                (channel_id, ticket_type, ticket_id, last_message_at,
                 last_fight_ping_at, extension_offered_at, extension_expires_at)
            VALUES (?, ?, ?, CURRENT_TIMESTAMP, NULL, NULL, NULL)
        """, (channel_id, ticket_type, ticket_id))
        await db.commit()


async def update_ticket_last_message(channel_id: int):
    """
    Called whenever a real user message lands in a ticket channel.
    Resets the inactivity clock and clears any pending auto-close warning.
    """
    async with aiosqlite.connect(DB) as db:
        await db.execute("""
            UPDATE ticket_activity
            SET last_message_at = CURRENT_TIMESTAMP,
                extension_offered_at = NULL,
...
async def get_ticket_activity(channel_id: int):
    async with aiosqlite.connect(DB) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM ticket_activity WHERE channel_id=?", (channel_id,)
        ) as cursor:
            return await cursor.fetchone()


async def set_fight_ping_sent(channel_id: int):
    """Records the timestamp of the most recent 3-day fight reminder ping."""
    async with aiosqlite.connect(DB) as db:
        await db.execute(
            "UPDATE ticket_activity SET last_fight_ping_at = CURRENT_TIMESTAMP WHERE channel_id=?",
            (channel_id,)
        )
        await db.commit()


async def set_extension_offered(channel_id: int, expires_at_iso: str):
    """Marks that the 48-hour close-warning embed has been posted."""
    async with aiosqlite.connect(DB) as db:
        await db.execute("""
            UPDATE ticket_activity
            SET extension_offered_at = CURRENT_TIMESTAMP,
                extension_expires_at = ?
            WHERE channel_id = ?
        """, (expires_at_iso, channel_id))
        await db.commit()


async def clear_extension_offer(channel_id: int):
    """User clicked 'Keep Open' — reset all inactivity timers."""
    async with aiosqlite.connect(DB) as db:
        await db.execute("""
            UPDATE ticket_activity
            SET extension_offered_at = NULL,
                extension_expires_at = NULL,
...
async def get_all_active_tickets():
    """Returns every row in ticket_activity — used by the background task."""
    async with aiosqlite.connect(DB) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM ticket_activity") as cursor:
            return await cursor.fetchall()


async def delete_ticket_activity(channel_id: int):
    """Remove tracking row when ticket channel is deleted."""
    async with aiosqlite.connect(DB) as db:
        await db.execute("DELETE FROM ticket_activity WHERE channel_id=?", (channel_id,))
        await db.commit()
